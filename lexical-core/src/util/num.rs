...
macro_rules! float_method {
    ($f:ident, $t:tt, $meth:ident, $intr:ident, $libm:ident $(,$i:expr)*) => ({
        ...
        #[cfg(all(not(feature = "std"), not(feature = "use_libm")))]
        return unsafe { core::intrinsics::$intr($f $(,$i)*) };

        #[cfg(all(not(feature = "std"), feature = "use_libm"))]
...
macro_rules! float_method_msvc {
    ($f:ident, $ts:tt, $tl:tt, $meth:ident, $intr:ident, $libm:ident $(,$i:expr)*) => ({
        ...
        #[cfg(all(not(feature = "std"), not(target_env = "msvc"), not(feature = "use_libm")))]
        return unsafe { core::intrinsics::$intr($f $(,$i)*) };

        #[cfg(all(not(feature = "std"), target_env = "msvc", not(feature = "use_libm")))]
...
macro_rules! float_method_log_solaris {
    ($f:ident, $t:tt, $meth:ident, $intr:ident, $libm:ident $(,$i:expr)*) => ({
        ...
        #[cfg(all(not(feature = "std"), not(target_os = "solaris"), not(feature = "use_libm")))]
        return unsafe { core::intrinsics::$intr($f $(,$i)*) };

        #[cfg(all(not(feature = "std"), not(target_os = "solaris"), feature = "use_libm"))]
        return libm::$libm($f $(,$i)*);
...
impl Float for f64 {
    ...
    fn powi(self, n: i32) -> f64 {
        #[cfg(not(feature = "use_libm"))]
            {
                float_method!(self, f64, powi, powif64, pow, n)
            }
